...
# setting data in order of time for each user
...
# setting data in sequence format (windowing)
...
def create_sequences(df, y_data, seq_length=10):
    ...
    for i in range(len(df) - seq_length):
        ...
        y.append(y_data[i+seq_length]) # predicting the next step
...
def build_sequence_model(n_func, n_user, n_time, seq_len):
    ...
    # Embeddings to capture relationships in the data
...
for train_index, val_index in tscv.split(X_func):
    ...
    # Train/Val Split
    X_f_tr, X_f_val = X_func[train_index], X_func[val_index]
    X_u_tr, X_u_val = X_user[train_index], X_user[val_index]
    X_t_tr, X_t_val = X_time[train_index], X_time[val_index]
    y_tr, y_val = y_func[train_index], y_func[val_index]

    model = build_sequence_model(len(func_enc.classes_), len(user_enc.classes_), len(time_enc.classes_), sequence_length)

    focal_loss = CategoricalFocalCrossentropy(alpha=alpha_weights, gamma=3.0)
...
model.save('gru_focal_loss_model10_1.h5')

with open('func_encoder10_1.pkl', 'wb') as f:
    pickle.dump(func_enc, f)
with open('user_encoder10_1.pkl', 'wb') as f:
    pickle.dump(user_enc, f)
with open('time_encoder10_1.pkl', 'wb') as f:
