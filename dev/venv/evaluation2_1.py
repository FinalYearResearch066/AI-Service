...
def prepare_evaluation_data():
    ...
    model_path = os.path.join(base_dir, "gru_next_function_model5.h5")
    func_enc_path = os.path.join(base_dir, "func_encoder5.pkl")
    user_enc_path = os.path.join(base_dir, "user_encoder5.pkl")
    scaler_path = os.path.join(base_dir,"time_scaler5.pkl")
    ...
    # Handle unseen function labels by mapping to the most frequent class
    known_funcs = set(func_enc.classes_)
    unseen_funcs = df[~df['function'].astype(str).isin(known_funcs)]['function'].unique()
    if len(unseen_funcs) > 0:
        default_func = func_enc.classes_[0]
        print(f"[Info] Found {len(unseen_funcs)} unseen function(s): {list(unseen_funcs)[:5]}")
        print(f"[Info] Mapping unseen functions to default class: '{default_func}'")
        df.loc[~df['function'].astype(str).isin(known_funcs), 'function'] = default_func
