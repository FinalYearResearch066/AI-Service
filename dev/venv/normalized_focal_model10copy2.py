...
def create_sequences(df, y_data, seq_length=10):
    ...
    for i in range(len(df) - seq_length):
        ...
        X_s.append(df['status_idx'].iloc[i:i+seq_length].values)
        y.append(y_data[i+seq_length]) # ඊළඟ පියවර පුරෝකථනය කිරීමට
    return np.array(X_f), np.array(X_u), np.array(X_t), np.array(X_s), np.array(y)

sequence_length = 10
X_func, X_user, X_time, X_status, y_func = create_sequences(df, y_all, sequence_length)
...
def build_sequence_model(n_func, n_user, n_time, seq_len):
    ...
    input_s = Input(shape=(seq_len, 1), name="Status_Input")
    ...
    merged = Concatenate()([emb_f, emb_u, emb_t, input_s])
    ...
    model = Model(inputs=[input_f, input_u, input_t, input_s], outputs=output)
    return model
...
X_s_tr, X_s_val = X_status[:split_idx], X_status[split_idx:]
y_tr, y_val = y_func[:split_idx], y_func[split_idx:]

X_s_tr = X_s_tr.reshape((-1, sequence_length, 1)).astype(np.float32)
X_s_val = X_s_val.reshape((-1, sequence_length, 1)).astype(np.float32)
...
model.fit(
    [X_f_tr, X_u_tr, X_t_tr, X_s_tr], y_tr,
    validation_data=([X_f_val, X_u_val, X_t_val, X_s_val], y_val),
    epochs=30,
    batch_size=64,
...
model.save('gru_focal_loss_model10copy2.h5')

with open('func_encoder10copy2.pkl', 'wb') as f:
    pickle.dump(func_enc, f)
with open('user_encoder10copy2.pkl', 'wb') as f:
    pickle.dump(user_enc, f)
with open('time_encoder10copy2.pkl', 'wb') as f:
