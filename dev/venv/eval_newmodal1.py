...
def prepare_evaluation_data():
    ...
    func_labels = df['function'].astype(str)
    user_labels = df['user'].astype(str)
    func_to_idx = {label: idx for idx, label in enumerate(func_enc.classes_)}
    user_to_idx = {label: idx for idx, label in enumerate(user_enc.classes_)}

    df['func_idx'] = func_labels.map(func_to_idx)
    df['user_idx'] = user_labels.map(user_to_idx)

    unknown_func_mask = df['func_idx'].isna()
    unknown_user_mask = df['user_idx'].isna()
    unknown_mask = unknown_func_mask | unknown_user_mask
    if unknown_mask.any():
        unknown_func_examples = func_labels[unknown_func_mask].drop_duplicates().head(5).tolist()
        unknown_user_examples = user_labels[unknown_user_mask].drop_duplicates().head(5).tolist()
        print(
            "[Warn] Dropping rows with unseen labels: "
            f"count={int(unknown_mask.sum())}, "
            f"unknown_functions={unknown_func_examples}, "
            f"unknown_users={unknown_user_examples}"
        )
        df = df.loc[~unknown_mask].copy()

    if df.empty:
        raise ValueError("All rows were filtered out due to unseen function/user labels.")

    df['func_idx'] = df['func_idx'].astype(np.int32)
...
def plot_evaluation_graphs(y_true, y_pred, y_prob, func_enc):
    ...
    plt.tight_layout()
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "eval_newmodal1_4charts2.png")
