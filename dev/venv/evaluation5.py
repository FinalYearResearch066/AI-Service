...
def calculate_train_val_metrics(model, x_func, x_tau, y_true, num_classes, y_interval=None):
    try:
        if y_interval is None:
            split = train_test_split(
                x_func, x_tau, y_true, test_size=0.2, random_state=42, stratify=y_true
            )
            x_func_train, x_func_val, x_tau_train, x_tau_val, y_train, y_val = split
            y_int_train, y_int_val = None, None
        else:
            split = train_test_split(
                x_func, x_tau, y_true, y_interval, test_size=0.2, random_state=42, stratify=y_true
            )
            (
                x_func_train, x_func_val,
                x_tau_train, x_tau_val,
                y_train, y_val,
        ...
        if y_interval is None:
            split = train_test_split(
                x_func, x_tau, y_true, test_size=0.2, random_state=42, stratify=None
            )
            x_func_train, x_func_val, x_tau_train, x_tau_val, y_train, y_val = split
            y_int_train, y_int_val = None, None
        else:
            split = train_test_split(
                x_func, x_tau, y_true, y_interval, test_size=0.2, random_state=42, stratify=None
            )
            (
                x_func_train, x_func_val,
                x_tau_train, x_tau_val,
    ...
    train_out = model.predict([x_func_train, x_tau_train], verbose=0)
    val_out = model.predict([x_func_val, x_tau_val], verbose=0)
...
def prepare_evaluation_data():
    ...
    model_path = os.path.join(base_dir, "neural_tpp_model.h5")
    interval_scaler_path = os.path.join(base_dir, "tau_scaler.pkl")
    ...
    df['interval_scaled'] = interval_scaler.transform(df[['tau']])

    x_func, x_tau, y_true, y_interval_true = [], [], [], []
    for user_id, user_df in df.groupby('user_idx'):
        ...
        for i in range(len(user_df) - sequence_len):
            ...
            x_tau.append(interval_seq[i:i + sequence_len])
    ...
    x_tau = np.expand_dims(np.array(x_tau), axis=-1)
    y_true = np.array(y_true)
    y_interval_true = np.array(y_interval_true)

    pred_out = model.predict([x_func, x_tau], verbose=0)
    y_prob, y_interval_pred = extract_model_outputs(pred_out)
    y_pred = np.argmax(y_prob, axis=1)
    return model, x_func, x_tau, y_true, y_interval_true, y_interval_pred, y_pred, y_prob, func_enc


if __name__ == "__main__":
    model, x_func, x_tau, y_true, y_interval_true, y_interval_pred, y_pred, y_prob, func_enc = prepare_evaluation_data()
    calculate_train_val_metrics(model, x_func, x_tau, y_true, len(func_enc.classes_), y_interval_true)
